...
import iijiaban.foodtherapy.ui.materialnavigation.CookDetailActivity;
...
public class CookSearchFragment extends Fragment{
	...
	private List<CookItem> foodItems = new ArrayList<CookItem>();
	...
	void getData(){  
		new AsyncTask<Void, Integer, List<CookItem>>() {
			@Override
			protected List<CookItem> doInBackground(Void... params) {
				// TODO 网络任务
				...
				try {
					...
					list = SearchDao.getCookSearchItem(json);
				} catch (IOException e) {
					// TODO 网络错误
					e.printStackTrace();
				} 
			...
			protected void onPostExecute(List<CookItem> result) {
				isloading = false;
	...
	void initAdapter(){
		adapter = new CommonViewAdapter(foodItems,getActivity()) {
			...
			public View setView(View convertView, int position, ViewGroup parent) {
				...
				.load(((CookItem)common_datas.get(position)).getImg()).into(viewholder.imageview);
				viewholder.txt_title.setText(Html.fromHtml(((CookItem)common_datas.get(position)).getName()));
				viewholder.txt_content.setText(Html.fromHtml(((CookItem)common_datas.get(position)).getContent()));
				return convertView;
			} 
			@Override
