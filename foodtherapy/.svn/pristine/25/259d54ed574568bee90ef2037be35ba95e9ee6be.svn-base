...
import iijiaban.foodtherapy.ui.materialnavigation.FoodDetailActivity;
...
public class SearchFragment extends Fragment {
	...
	void getData(){  
		new AsyncTask<Void, Integer, List<FoodItem>>() {
			@Override
			protected List<FoodItem> doInBackground(Void... params) {
				...
				try {
					...
					list = SearchDao.getFoodSearchItem(json);
				} catch (IOException e) {
					// TODO 网络错误
					e.printStackTrace();
				} 
	...
	void initAdapter(){
		adapter = new CommonViewAdapter(foodItems,getActivity()) {
			...
			public View setView(View convertView, int position, ViewGroup parent) {
				...
				.load(((FoodItem)common_datas.get(position)).getImg()).into(viewholder.imageview);
				viewholder.txt_title.setText(Html.fromHtml(((FoodItem)common_datas.get(position)).getTitle()));
				viewholder.txt_content.setText(Html.fromHtml(((FoodItem)common_datas.get(position)).getContent()));
				return convertView;
			} 
			@Override
